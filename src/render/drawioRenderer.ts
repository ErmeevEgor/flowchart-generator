import type { LayoutNode, ProcessLayout } from "../model/processModel.js";

export function renderDrawio(layout: ProcessLayout): string {
  const cells: string[] = [
    '<mxCell id="0"/>',
...
function portStyle(point: { x: number; y: number }, node: LayoutNode, prefix: "exit" | "entry"): string {
  const { x, y, width, height } = node.bounds;
  const distances = [
...
function nodeStyle(node: LayoutNode): string {
  ...
  if (node.type === "decision") return `${common}shape=rhombus;perimeter=rhombusPerimeter;fillColor=#fffaeb;strokeColor=#b54708;`;
  if (node.type === "start" || node.type === "end") return `${common}rounded=1;arcSize=40;fillColor=#eef4ff;strokeColor=#3538cd;`;
  if (node.type === "boundary") return `${common}rounded=1;arcSize=25;dashed=1;fillColor=#f2f4f7;strokeColor=#475467;`;
...
function nodeValue(node: LayoutNode): string {
  ...
  if (node.system) details.push(node.system);
  if (node.document) details.push(`«${node.document}»`);
  if (node.subprocess) details.push(`Подпроцесс: ${node.subprocess}`);
