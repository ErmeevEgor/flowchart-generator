import type { LayoutNode, Point, ProcessEdge, ProcessLayout, ProcessModel, RoutedEdge } from "../model/processModel.js";
import type { NodeLayoutResult } from "../layout/layoutEngine.js";
...
export function routeEdges(model: ProcessModel, nodeLayout: NodeLayoutResult, options: RoutingOptions = DEFAULT_ROUTING): ProcessLayout {
  ...
  model.edges.forEach((edge, index) => {
    ...
    const routedEdge: RoutedEdge = { ...edge, id: edge.id ?? `e${index + 1}`, points };
    if (edge.label) routedEdge.labelPoint = findLabelPoint(points);
...
export function rerouteEdges(model: ProcessModel, nodeLayout: NodeLayoutResult, options?: RoutingOptions): ProcessLayout {
  return routeEdges(model, nodeLayout, options);
}

function forwardCandidates(source: LayoutNode, target: LayoutNode, nodes: LayoutNode[], maxBottom: number, options: RoutingOptions, startOffset: number, endOffset: number): Point[][] {
  if (target.lane !== source.lane) return branchCandidates(source, target, nodes, maxBottom, options, startOffset, endOffset);
  ...
  if (Math.abs(start.y - end.y) < 0.01) candidates.push([start, end]);
...
function branchCandidates(source: LayoutNode, target: LayoutNode, nodes: LayoutNode[], maxBottom: number, options: RoutingOptions, startOffset: number, endOffset: number): Point[][] {
  ...
  const candidates: Point[][] = [];
  if (Math.abs(start.x - end.x) < 0.01) candidates.push([start, end]);
...
function returnCandidates(source: LayoutNode, target: LayoutNode, nodes: LayoutNode[], maxBottom: number, options: RoutingOptions, index: number): Point[][] {
...
function selectRoute(candidates: Point[][], source: LayoutNode, target: LayoutNode, nodes: LayoutNode[], routed: RoutedEdge[], clearance: number): Point[] {
...
function countObstacleHits(points: Point[], sourceId: string, targetId: string, nodes: LayoutNode[], clearance: number): number {
  let count = 0;
  for (let i = 0; i < points.length - 1; i += 1) {
    for (const node of nodes) {
      if (node.id === sourceId || node.id === targetId) continue;
      if (segmentIntersectsRect(points[i]!, points[i + 1]!, node.bounds, clearance)) count += 1;
    }
  }
  return count;
}

function countExistingEdgeHits(points: Point[], edges: RoutedEdge[]): number {
  let hits = 0;
  for (const edge of edges) {
    for (let a = 0; a < points.length - 1; a += 1) {
      for (let b = 0; b < edge.points.length - 1; b += 1) {
        if (segmentsCross(points[a]!, points[a + 1]!, edge.points[b]!, edge.points[b + 1]!)) hits += 1;
      }
    }
  }
  return hits;
...
function pathLength(points: Point[]): number {
  let result = 0;
  for (let i = 0; i < points.length - 1; i += 1) result += Math.abs(points[i + 1]!.x - points[i]!.x) + Math.abs(points[i + 1]!.y - points[i]!.y);
  return result;
}

function findLabelPoint(points: Point[]): Point {
...
function topAnchor(node: LayoutNode, offset = 0): Point {
...
function bottomAnchor(node: LayoutNode, offset = 0): Point {
...
function columnRight(rank: number, nodes: LayoutNode[]): number {
  return Math.max(...nodes.filter((node) => node.rank === rank).map((node) => node.bounds.x + node.bounds.width));
}

function columnLeft(rank: number, nodes: LayoutNode[]): number {
