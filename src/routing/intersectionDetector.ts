import type { Point, RoutedEdge } from "../model/processModel.js";
...
export function detectEdgeIntersections(edges: RoutedEdge[]): EdgeIntersection[] {
  const result: EdgeIntersection[] = [];
  for (let i = 0; i < edges.length; i += 1) {
    ...
    for (let j = i + 1; j < edges.length; j += 1) {
      ...
      let found: EdgeIntersection | undefined;
...
function orthogonalIntersection(a1: Point, a2: Point, b1: Point, b2: Point): { point: Point; overlap: boolean } | undefined {
  ...
  if (aVertical && a1.x === b1.x) {
    ...
    if (low <= high) return { point: { x: a1.x, y: (low + high) / 2 }, overlap: high > low };
  }
  if (!aVertical && a1.y === b1.y) {
    ...
    if (low <= high) return { point: { x: (low + high) / 2, y: a1.y }, overlap: high > low };
  }
  return undefined;
}
