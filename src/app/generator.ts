...
import type { Diagnostic, ProcessBundle, ProcessLayout, ProcessModel } from "../model/processModel.js";
...
export interface GenerationResult {
  ...
  files?: { model: string; drawio: string; svg: string; layout: string };
}

export interface ProcessBundleGenerationResult {
  root: string;
  manifest: string;
  overview: GenerationResult;
  subprocesses: Array<{ id: string; overviewNodeId: string; result: GenerationResult }>;
}
...
export async function generateProcessDiagram(model: ProcessModel, options: GenerationOptions = {}): Promise<GenerationResult> {
  const graphValidation = validateGraph(model);
  if (!graphValidation.valid) throw new Error(formatDiagnostics("Invalid Process Model", graphValidation.diagnostics));
  ...
  if (!layoutValidation.valid) throw new Error(formatDiagnostics("Invalid diagram layout", layoutValidation.diagnostics));

  if (!options.outputDirectory) return { layout, diagnostics };
...
export async function generateProcessBundle(
  bundle: ProcessBundle,
  options: Omit<GenerationOptions, "outputDirectory"> & { outputDirectory: string },
): Promise<ProcessBundleGenerationResult> {
  ...
  const overviewModel: ProcessModel = {
    ...bundle.overview,
  ...
  const subprocesses: ProcessBundleGenerationResult["subprocesses"] = [];
...
function formatDiagnostics(title: string, diagnostics: Diagnostic[]): string {
...
function sanitizeSubprocessId(value: string): string {
  const result = value.replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").toLocaleLowerCase();
  return result || "subprocess";
}

function manifestFiles(root: string, files: NonNullable<GenerationResult["files"]>, includeLayout: boolean): Record<string, string> {
  return {
