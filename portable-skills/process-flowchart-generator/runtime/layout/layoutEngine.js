...
export function layoutProcess(model, options = DEFAULT_LAYOUT) {
...
function findHappyPath(model, outgoing) {
    ...
    while (current && !seen.has(current.id)) {
        ...
        const nextId = choices[0]?.edge.to;
        current = nextId ? model.nodes.find((node) => node.id === nextId) : undefined;
    }
    return path;
}
function edgePriority(edge, index) {
    if (edge.kind === "main")
        return index;
    if (/^(да|yes|основн)/iu.test(edge.label ?? ""))
        return 10_000 + index;
    if (edge.kind === "alternative")
        return 30_000 + index;
    return 20_000 + index;
}
function findBackEdges(model, outgoing) {
...
function assignRanks(model, backEdges) {
    const incoming = new Map(model.nodes.map((node) => [node.id, 0]));
    ...
    for (const node of model.nodes)
        if ((incoming.get(node.id) ?? 0) === 0)
            rank.set(node.id, 0);
    if (rank.size === 0 && model.nodes[0])
        rank.set(model.nodes[0].id, 0);
    for (let pass = 0; pass < model.nodes.length; pass += 1) {
        let changed = false;
        model.edges.forEach((edge, index) => {
            if (backEdges.has(index) || !rank.has(edge.from))
                return;
...
function assignLanes(model, happyPath, happySet, outgoing, backEdges) {
    const lanes = new Map();
    ...
    for (const happyId of happyPath) {
        ...
        for (const item of outgoing.get(happyId) ?? []) {
            if (item.edge.to === mainNext || happySet.has(item.edge.to) || backEdges.has(item.index))
                continue;
            const lane = nextLane++;
            const queue = [item.edge.to];
            while (queue.length > 0) {
                const id = queue.shift();
                if (happySet.has(id) || lanes.has(id))
                    continue;
                lanes.set(id, lane);
                for (const child of outgoing.get(id) ?? [])
                    if (!backEdges.has(child.index))
                        queue.push(child.edge.to);
            }
        }
    }
    ...
    model.edges.forEach((edge, index) => {
        if (!backEdges.has(index))
            forwardIncoming.set(edge.to, (forwardIncoming.get(edge.to) ?? 0) + 1);
    });
    for (const node of model.nodes) {
        ...
        for (const alternative of choices.slice(1)) {
            if ((lanes.get(alternative.edge.to) ?? lanes.get(node.id) ?? 0) !== (lanes.get(node.id) ?? 0))
                continue;
            const lane = nextLane++;
            ...
            while (queue.length > 0) {
                const id = queue.shift();
                if (visited.has(id) || happySet.has(id))
                    continue;
                visited.add(id);
                if (id !== seed && (forwardIncoming.get(id) ?? 0) > 1)
                    continue;
                lanes.set(id, lane);
                for (const child of outgoing.get(id) ?? [])
                    if (!backEdges.has(child.index))
...
function makeRankLanePairsUnique(nodes, ranks, lanes) {
    const used = new Set();
    let maxLane = Math.max(0, ...lanes.values());
    for (const node of nodes) {
        const rank = ranks.get(node.id) ?? 0;
        let lane = lanes.get(node.id) ?? 0;
        while (used.has(`${rank}:${lane}`))
            lane = ++maxLane;
