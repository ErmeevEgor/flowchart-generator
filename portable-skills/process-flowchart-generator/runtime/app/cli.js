...
async function main() {
    ...
    if (options.mode === "process_with_subprocesses") {
        ...
        const bundle = parseProcessBundle(JSON.parse(source));
    ...
    const model = extname(inputPath).toLocaleLowerCase() === ".json"
        ? parseProcessModel(JSON.parse(source))
    ...
    if (options.layout)
        console.log(`Layout JSON:   ${result.files.layout}`);
...
Options:
  ...
  -h, --help         Show this help`);
    ...
    const result = { input, output, layout, mode };
    if (name)
        result.name = name;
    if (title)
        result.title = title;
