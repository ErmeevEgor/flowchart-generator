import { NODE_TYPES } from "../model/processModel.js";
export function validateGraph(model) {
    ...
    for (const node of model.nodes) {
        ...
        ids.add(node.id);
        if (node.text.trim() === "")
            diagnostics.push(error("EMPTY_NODE_TEXT", `Node '${node.id}' has empty text.`, [node.id]));
        if (!NODE_TYPES.includes(node.type))
            diagnostics.push(error("UNSUPPORTED_NODE_TYPE", `Node '${node.id}' has unsupported type.`, [node.id]));
    }
    const outgoing = new Map();
    const incomingCount = new Map(model.nodes.map((node) => [node.id, 0]));
    for (const edge of model.edges) {
        if (!ids.has(edge.from))
            diagnostics.push(error("UNKNOWN_EDGE_SOURCE", `Edge source '${edge.from}' does not exist.`, [edge.from]));
        if (!ids.has(edge.to))
            diagnostics.push(error("UNKNOWN_EDGE_TARGET", `Edge target '${edge.to}' does not exist.`, [edge.to]));
        if (edge.from === edge.to)
            diagnostics.push(error("SELF_LOOP", `Self-loop on '${edge.from}' is not allowed.`, [edge.from]));
    ...
    for (const node of model.nodes) {
        ...
        if (node.type === "decision") {
            ...
            for (const edge of routes) {
                if (!edge.label?.trim())
                    diagnostics.push(error("UNLABELLED_DECISION_EDGE", `Every route from decision '${node.id}' must have a label.`, [node.id, edge.to]));
            }
        }
...
function error(code, message, elements) {
    const diagnostic = { severity: "error", code, message };
    if (elements)
        diagnostic.elements = elements;
    return diagnostic;
}
